...
else: logger.warning(f"Logo file NOT FOUND at {LOGO_PATH_SIDEBAR}")
...
uploaded_file = st.sidebar.file_uploader("Upload Trading Journal (CSV)", type=["csv"], key="app_wide_file_uploader_auth")
        ...
        with st.spinner("Calculating metrics..."):
            ...
            if kpi_res and 'error' not in kpi_res:
                ...
                if pnl_col_ci and pnl_col_ci in st.session_state.filtered_data:
                    ...
                    else: st.session_state.kpi_confidence_intervals = {}; logger.info(f"Not enough PnL data ({len(pnl_s_ci)}) for CI.")
                else: st.session_state.kpi_confidence_intervals = {}; logger.info("PnL col for CI not found.")
            else:
...
def main_page_layout():
    ...
    with col1: st.markdown("<div class='feature-item'><h4>📄 Upload Data</h4><p>Upload CSV via sidebar.</p></div>", unsafe_allow_html=True)
    with col2: st.markdown("<div class='feature-item'><h4>📊 Analyze</h4><p>Explore performance metrics.</p></div>", unsafe_allow_html=True)
    ...
    if os.path.exists(user_guide_page_path):
        if st.button("📘 Read User Guide", key="welcome_guide_btn_auth"): st.switch_page(user_guide_page_path)
