...
# --- NEW: Asset Paths ---
LOGO_PATH_FOR_BROWSER_TAB: str = "assets/Trading_Mastery_Hub_600x600.png" # Or your preferred icon path
LOGO_PATH_SIDEBAR: str = "assets/Trading_Mastery_Hub_600x600.png" # Can be the same or different
...
CONCEPTUAL_COLUMN_SYNONYMS: Dict[str, List[str]] = {
    "date": ["trade date", "timestamp", "execution time", "time"],
    "pnl": ["profit", "loss", "net pnl", "profit_loss", "realized_pnl"],
    "symbol": ["ticker", "instrument", "market", "asset"],
    "strategy": ["strategy_name", "model_name", "algo_id", "trade_model"],
    "trade_id": ["order_id", "execution_id", "transaction_id"],
    "entry_price": ["open_price", "buy_price", "sell_price"],
    "exit_price": ["close_price"],
    "risk_pct": ["risk_percentage", "risk %"],
    "notes": ["comments", "description", "trade_rationale", "lesson_learned"],
    "duration_minutes": ["trade_duration_min", "holding_period_min", "duration_mins"],
    "trade_size_num": ["quantity", "volume", "contracts", "shares", "lots", "size"],
    "r_r_csv_num": ["r:r", "rr_ratio", "risk_reward"],
    "direction_str": ["direction", "side", "long_short", "trade_direction"],
    "account_str": ["account_id", "account_name", "portfolio_id"],
    # Add more synonyms as needed for other conceptual columns
}
CRITICAL_CONCEPTUAL_COLUMNS: List[str] = ["date", "pnl"] # Symbol could also be critical depending on use case

# Define categories for the column mapper UI
# Using collections.OrderedDict to maintain insertion order for display
CONCEPTUAL_COLUMN_CATEGORIES: OrderedDict[str, List[str]] = OrderedDict([
    ("Critical Trade Info", ["date", "pnl", "symbol", "trade_id"]),
    ("Trade Execution Details", ["entry_price", "exit_price", "trade_size_num", "direction_str", "entry_time_str"]),
    ...
    ("Indicators & Signals", ["signal_confidence", "loss_indicator_num", "win_indicator_num"]),
    ("Temporal & Duration", ["duration_minutes", "duration_hrs_csv", "candle_count_num", "trade_day_of_week_str", "trade_month_str"]),
    ("Account & Balance", ["account_str", "initial_balance_num", "current_balance_num"]),
    ("Events & Compliance", ["event_type_str", "events_details_str", "event_release_time", "market_reaction_on_news", "compliance_check_str"]),
    ("Planning & Screenshots", ["planned_entry_price", "planned_sl_price", "planned_tp_price", "planned_rrr", "pre_market_emotions", "pre_market_notes", "trade_plan_screenshot", "trade_result_screenshot"]),
    ("Other Numeric/Categorical", ["multiplier_value", "absolute_daily_pnl_csv", "error_exit_type_related_str", "htf_key_level_focus", "trade_outcome_csv_str"])
])
...
PLOT_BG_COLOR_DARK = '#0E1117' # Match theme.backgroundColor
PLOT_PAPER_BG_COLOR_DARK = '#0E1117' # Match theme.backgroundColor
PLOT_FONT_COLOR_DARK = '#FAFAFA' # Match theme.textColor
...
# --- KPI Groupings & Order ---
# Define the order and grouping for displaying KPIs on the Overview page.
# Uses the programmatic keys from kpi_definitions.py
KPI_GROUPS_OVERVIEW: OrderedDict[str, List[str]] = OrderedDict([
    ...
    ("Benchmark Comparison", [ # Populated if benchmark is active
        "benchmark_total_return", "alpha", "beta",
        "benchmark_correlation", "tracking_error", "information_ratio"
    ])
])

# For pages that might show a smaller, more focused set of KPIs
...
KPI_GROUPS_RISK_DURATION: OrderedDict[str, List[str]] = OrderedDict([
    ...
    ("Market Risk & Relative Performance", [ # Benchmark related risk metrics
...
DISTRIBUTIONS_TO_FIT: List[str] = ['norm', 't', 'laplace', 'cauchy', 'skewnorm'] # Added skewnorm
...
# --- EXPECTED_COLUMNS ---
# This maps conceptual keys to the actual column names that will exist in the DataFrame
# *after* data_processing.py has applied the user's mapping and renamed columns
# to these conceptual keys. So, typically, key and value will be the same here.
...
# Re-evaluate DATABASE_URL and other env-dependent vars after load_dotenv()
DATABASE_TYPE = os.getenv("DATABASE_TYPE", "sqlite") # Re-read after dotenv
...
else: # Fallback if DATABASE_TYPE is something else unexpected
    print(f"Warning (config.py): Unknown DATABASE_TYPE '{DATABASE_TYPE}'. Defaulting to SQLite.")
    DATABASE_URL = f"sqlite:///{SQLITE_DB_FILE}"

if not DATABASE_URL: # Should not happen with fallbacks, but as a final check
    print("CRITICAL WARNING (config.py): DATABASE_URL is still not set after all checks. Defaulting to an in-memory SQLite DB for this session only.")
    DATABASE_URL = "sqlite:///:memory:"


APP_BASE_URL: str = os.getenv("APP_BASE_URL", "http://localhost:8501")
LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO").upper() # Re-read log level
EMAIL_SENDER_ADDRESS = os.getenv("EMAIL_SENDER_ADDRESS", "noreply@yourtradinghub.com")
PASSWORD_RESET_LINK_EXPIRY_HOURS = int(os.getenv("PASSWORD_RESET_LINK_EXPIRY_HOURS", "1"))
