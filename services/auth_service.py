    ...
    def authenticate_user(self, username: str, password: str) -> Optional[User]:
        ...
        if db:
            try:
                ...
                if user_in_session:
                    user_in_session.last_login_at = dt.datetime.utcnow(); db.commit(); db.refresh(user_in_session)
                ...
                db.rollback(); self.logger.error(f"Error updating last_login_at for '{username}': {e}", exc_info=True)
                return user
            finally:
                if db: db.close()
        else:
