...
try:
    ...
    from .database_setup import Base, get_db_session, UserFile, UserFileMapping # Import UserFileMapping
    ...
    def load_and_process_data(uploaded_file_obj: Any, user_column_mapping: Optional[Dict[str, str]] = None, original_file_name: Optional[str] = None) -> Optional[pd.DataFrame]: # Added original_file_name
        if uploaded_file_obj:
            try: return pd.read_csv(uploaded_file_obj)
            except: return None
        return None
    class UserFile: pass # Placeholder
    class UserFileMapping: pass # Placeholder
...
class TradeNoteDB(Base): # type: ignore
    ...
    user_id = Column(Integer, ForeignKey("users.id", name="fk_trade_notes_user_id"), nullable=False, index=True) # Added user_id
    trade_identifier = Column(String, index=True, nullable=True) # Link to trade_id in CSV
    note_timestamp = Column(DateTime, default=dt.datetime.utcnow, nullable=False)
    note_content = Column(Text, nullable=False)
    tags = Column(String, nullable=True)

    def __repr__(self):
        return f"<TradeNoteDB(id={self.id}, user_id={self.user_id}, trade_identifier='{self.trade_identifier}')>"
...
def get_benchmark_data_static(
    ...
    logger_static_func = logging.getLogger(f"{APP_TITLE}.get_benchmark_data_static") # Ensure logger name is valid
    ...
    try:
        ...
        if start_dt == end_dt: fetch_end_dt = end_dt + pd.Timedelta(days=2) # Ensure yfinance gets data for single day
        ...
        if len(daily_adj_close) < 2: return None # Need at least two points for pct_change
...
class DataService:
    ...
    def get_processed_trading_data(
        ...
        original_file_name: Optional[str] = None # Added to pass to load_and_process_data
        ...
        try:
            ...
            processed_df = load_and_process_data(
                ...
                original_file_name=file_name_for_log # Pass it through
            )
    ...
    # --- Trade Note CRUD Methods (Updated for user_id) ---
    def add_trade_note(self, user_id: int, trade_identifier: Optional[str], note_content: str, tags: Optional[str] = None) -> Optional[TradeNoteDB]:
        ...
        try:
            ...
            db.rollback()
            self.logger.error(f"Error adding note for user_id {user_id}: {e}", exc_info=True)
            return None
        finally:
            if db: db.close()
    ...
    def get_trade_notes(self, user_id: int, trade_identifier: Optional[str]=None, start_date: Optional[dt.datetime]=None, end_date: Optional[dt.datetime]=None, limit: Optional[int]=None) -> List[TradeNoteDB]:
        ...
        if not db: return []
        try:
            query = db.query(TradeNoteDB).filter(TradeNoteDB.user_id == user_id) # Filter by user_id
            ...
            self.logger.error(f"Error getting notes for user_id {user_id}: {e}", exc_info=True)
            return []
        finally:
            if db: db.close()
    ...
    def update_trade_note(self, user_id: int, note_id: int, new_content: Optional[str]=None, new_tags: Optional[str]=None) -> Optional[TradeNoteDB]:
        ...
        if not db: return None
        try:
            note = db.query(TradeNoteDB).filter(TradeNoteDB.id == note_id, TradeNoteDB.user_id == user_id).first() # Ensure user owns note
            if not note:
                self.logger.warning(f"Note ID {note_id} not found for user_id {user_id} during update.")
                return None
            updated = False
            ...
            db.rollback()
            self.logger.error(f"Error updating note {note_id} for user_id {user_id}: {e}", exc_info=True)
            return None
        finally:
            if db: db.close()
    ...
    def delete_trade_note(self, user_id: int, note_id: int) -> bool:
        ...
        try:
            note = db.query(TradeNoteDB).filter(TradeNoteDB.id == note_id, TradeNoteDB.user_id == user_id).first() # Ensure user owns note
            if not note:
                self.logger.warning(f"Note ID {note_id} not found for user_id {user_id} for deletion.")
                return False
            ...
            db.rollback()
            self.logger.error(f"Error deleting note {note_id} for user_id {user_id}: {e}", exc_info=True)
            return False
        finally:
            if db: db.close()

    # --- UserFile Handling Methods ---
    ...
    # --- UserFileMapping Methods ---
    def save_user_column_mapping(self, user_id: int, user_file_id: int, mapping: Dict[str, str]) -> bool:
        """Saves or updates column mappings for a specific user and file."""
        db = self._get_db()
        if not db: return False
        try:
            # Delete existing mappings for this user_file_id to replace with new ones
                ...
                if csv_col_name: # Only save if a CSV column is actually selected
                    new_mappings_to_add.append(
                        UserFileMapping(
                            ...
                            conceptual_column=conceptual_col,
                            csv_column_name=csv_col_name
                        )
                    )
            if new_mappings_to_add:
                db.add_all(new_mappings_to_add)
            
            db.commit()
            self.logger.info(f"Saved/Updated {len(new_mappings_to_add)} column mappings for user_id {user_id}, user_file_id {user_file_id}.")
            return True
        except SQLAlchemyError as e:
            db.rollback()
            self.logger.error(f"Error saving column mapping for user_id {user_id}, user_file_id {user_file_id}: {e}", exc_info=True)
            return False
        finally:
            if db: db.close()

    def get_user_column_mapping(self, user_id: int, user_file_id: int) -> Optional[Dict[str, str]]:
        """Retrieves the saved column mapping for a specific user and file."""
        db = self._get_db()
        if not db: return None
        try:
            mappings_db = db.query(UserFileMapping).filter_by(user_id=user_id, user_file_id=user_file_id).all()
            if not mappings_db:
                self.logger.info(f"No saved column mapping found for user_id {user_id}, user_file_id {user_file_id}.")
                return None
            
            loaded_mapping = {item.conceptual_column: item.csv_column_name for item in mappings_db}
            self.logger.info(f"Retrieved column mapping for user_id {user_id}, user_file_id {user_file_id}: {len(loaded_mapping)} items.")
            return loaded_mapping
        except SQLAlchemyError as e:
            self.logger.error(f"Error retrieving column mapping for user_id {user_id}, user_file_id {user_file_id}: {e}", exc_info=True)
            return None
        finally:
            if db: db.close()
